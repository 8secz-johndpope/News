...
public class classVO implements SOAPObject, Serializable
{
    ...
    public java.util.List<classSubVO> _classInfoSubList = null;
    ...
    public String _MILEAGE_DESCRIBE = null;


    public String yStorage;
    ...
    public classVO otherCabinClassVO;// 相对产品仓位的正常公布运价
    ...
    public void addElementsToNode(XmlSerializer xml) throws IOException
    {
        ...
        if(_classInfoSubList != null) {
        	if(_classInfoSubList.size()>0){
        		for(int i=0,len=_classInfoSubList.size();i<len;i++){
                    xml.startTag(null, "classInfoSubList");
                    _classInfoSubList.get(i).addElementsToNode(xml);
                    xml.endTag(null, "classInfoSubList");
        		}
        	}
        }
        if(_CLASS_NAME != null) {
        ...
        if(_isSpPrice != null) {
            xml.startTag(null, "isSpPrice");
            xml.text(_isSpPrice);
            xml.endTag(null, "isSpPrice");
        }
        if(_LOWEST_ACC != null) {
        ...
        if(_ORIGINAL_PRODUCT_VALUE != null) {
            xml.startTag(null, "ORIGINAL_PRODUCT_VALUE");
            xml.text(_ORIGINAL_PRODUCT_VALUE);
            xml.endTag(null, "ORIGINAL_PRODUCT_VALUE");
        }
        ...
        if(_productInfoList != null) {
        	if(_productInfoList.size()>0){
        		for(int i=0,len=_productInfoList.size();i<len;i++){
                    xml.startTag(null, "productInfoList");
                    _productInfoList.get(i).addElementsToNode(xml);
                    xml.endTag(null, "productInfoList");
        		}
        	}
        }
        ...
        if(_RETRUN_FLAG != null) {
            xml.startTag(null, "RETRUN_FLAG");
            xml.text(_RETRUN_FLAG);
            xml.endTag(null, "RETRUN_FLAG");
        }
        if(_SAVE_MONEY != null) {
        ...
        if(_SEG_MILEAGE_POINTS != null) {
            ...
            xml.text(_SEG_MILEAGE_POINTS);
            xml.endTag(null, "SEG_MILEAGE_POINTS");
        }
        if(_spmealProductCode != null) {
            xml.startTag(null, "spmealProductCode");
            xml.text(_spmealProductCode);
            xml.endTag(null, "spmealProductCode");
        }
        if(_spmealPropertyType != null) {
            xml.startTag(null, "spmealPropertyType");
            xml.text(_spmealPropertyType);
            xml.endTag(null, "spmealPropertyType");
        }
        if(_STORGE != null) {
        ...
        if(_TICKET_PRICE != null) {
            xml.startTag(null, "TICKET_PRICE");
            xml.text(_TICKET_PRICE);
        ...
        if(_MILEAGE_DESCRIBE != null) {
            xml.startTag(null, "MILEAGE_DESCRIBE");
            xml.text(_MILEAGE_DESCRIBE);
            xml.endTag(null, "MILEAGE_DESCRIBE");
        }
		...
		try {
	        ...
	        while(event!= XmlPullParser.END_TAG){
	            ...
	            case XmlPullParser.START_TAG://判断当前事件是否是标签元素开始事件
	            	if("AFTER_COUPON_PRICE".equals(parser.getName())){
	            	...
	            	else if("classInfoSubList".equals(parser.getName())){
	            		if(_classInfoSubList==null){
	            			_classInfoSubList = new java.util.ArrayList<classSubVO>();
	            		}
	            		classSubVO soapObject = new classSubVO();
	                    soapObject.parse(binding, parser);
	                    _classInfoSubList.add(soapObject);
	            		break;
	            	}
	            	...
	            	else if("isSpPrice".equals(parser.getName())){
	            		_isSpPrice = parser.nextText();
	            		break;
	            	}
	            		...
	            		_MILEAGE_POINTS = parser.nextText();
	            		break;
	            	}
	            	...
	            	else if("ORIGINAL_PRODUCT_VALUE".equals(parser.getName())){
	            		_ORIGINAL_PRODUCT_VALUE = parser.nextText();
	            		break;
	            	}
	            	...
	            	else if("productInfoList".equals(parser.getName())){
	            		if(_productInfoList==null){
	            			_productInfoList = new java.util.ArrayList<productInfoVO>();
	            		}
	            		productInfoVO soapObject = new productInfoVO();
	                    soapObject.parse(binding, parser);
	                    _productInfoList.add(soapObject);
	            		break;
	            	}
	            	...
	            	else if("RETRUN_FLAG".equals(parser.getName())){
	            		_RETRUN_FLAG = parser.nextText();
	            		break;
	            	}
	            		...
	            		_SEG_MILEAGE_POINTS = parser.nextText();
	            		break;
	            	}
	            	else if("spmealProductCode".equals(parser.getName())){
	            		_spmealProductCode = parser.nextText();
	            		break;
	            	}
	            	else if("spmealPropertyType".equals(parser.getName())){
	            		_spmealPropertyType = parser.nextText();
	            		break;
	            	}
                    ...
                    else if("MILEAGE_DESCRIBE".equals(parser.getName())){
                        _MILEAGE_DESCRIBE = parser.nextText();
                        break;
                    }
