...
public class classListVO implements SOAPObject
{

    public java.util.List<classInfoVO> _CLASS_INFO = null;
    ...
    {
        if(_CLASS_INFO != null) {
        	if(_CLASS_INFO.size()>0){
        		for(int i=0,len=_CLASS_INFO.size();i<len;i++){
                    xml.startTag(null, "CLASS_INFO");
                    _CLASS_INFO.get(i).addElementsToNode(xml);
	        ...
	        while(event!= XmlPullParser.END_TAG){
	            ...
	            case XmlPullParser.START_TAG://判断当前事件是否是标签元素开始事件
	            	if("CLASS_INFO".equals(parser.getName())){
	            		if(_CLASS_INFO==null){
	            			_CLASS_INFO = new java.util.ArrayList<classInfoVO>();
	            		}
	            		classInfoVO soapObject = new classInfoVO();
	                    soapObject.parse(binding, parser);
	                    _CLASS_INFO.add(soapObject);
	            		break;
	            	}
	            	else{
