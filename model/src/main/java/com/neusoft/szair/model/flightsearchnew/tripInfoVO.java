...
public class tripInfoVO implements SOAPObject
{
    ...
    {
        if(_DEPARTURE_DATE != null) {
            xml.startTag(null, "DEPARTURE_DATE");
            xml.text(_DEPARTURE_DATE);
		...
		try {
	        ...
	        while(event!= XmlPullParser.END_TAG){
	            ...
	            case XmlPullParser.START_TAG://判断当前事件是否是标签元素开始事件
	            	if("DEPARTURE_DATE".equals(parser.getName())){
	            		_DEPARTURE_DATE = parser.nextText();
	            		break;
	            	}
